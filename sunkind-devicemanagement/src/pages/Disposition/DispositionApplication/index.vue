<template>
  <div class="DispositionApplication">
      ...
      <div>
        <ul>
          ...
          <li>
            ...
            >
              <a-select-option
                v-for="item in $state.dispositionStatus"
                :key="item.id"
                :value="item.id"
              >
                {{ item.value }}
    ...
    <div class="DispositionApplication-table">
      <div class="table-btn">
        <a-button
          class="primary-style"
          type="primary"
    ...
    <template>
      <template>
        <NewDialog
          ...
          @refresh="refresh"
        />
        <WithdrawDialog ref="withdrawDialog" @refresh="refresh" />
        <EditorDialog
          ref="editorDialog"
          :item="item"
          :devices="devices"
...
export default {
  ...
  methods: {
    ...
    getDispositionTableData() {
      ...
      this.spinning = true;
      getDispositionTable(params).then((res) => {
        console.log(res);
        this.spinning = false;
        if (res.status === 200) {
          if (res.data.rows && res.data.rows.length > 0) {
            res.data.rows.map((item) => {
              item.disposalWay = this.$utils.getStateName(
                "disposalWay",
                item.disposalWay
              );
    ...
    compile(item) {
      // console.log(item);
      this.item = item;
      this.$refs.withdrawDialog.afterVisibleChange(item);
    },
    /**
     * 删除处置
     */
