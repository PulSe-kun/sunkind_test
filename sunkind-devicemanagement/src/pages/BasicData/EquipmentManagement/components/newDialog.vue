...
<script>
import { getCmCodingIsAuto, getDepartments, addDevice } from "@/services/api";
export default {
  props: {
    treeData: {
      type: Array,
  ...
  methods: {
    afterVisibleChange(val) {
      ...
      if (val) {
        if (this.treeId !== "") {
          this.form.cmTypeId = this.treeId;
        }
        getCmCodingIsAuto().then((res) => {
          console.log(res);
          if (res.data) {
            this.disabled = true;
            this.rules.cmNum[0].required = false;
          } else {
            this.disabled = false;
            this.rules.cmNum[0].required = true;
          }
          this.$refs.ruleForm.clearValidate("cmNum");
        });
