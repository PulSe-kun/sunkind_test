    ...
    <a-tabs :default-active-key="activeKey" @change="callback">
      <a-tab-pane key="0">
        ...
        <div class="tab-content">
          ...
          <div class="InspectionTask-table">
...
import {
  getUsers_xj,
  getUsers_zb,
  getRemainingInspectionTable,
  getFinishedInspectionTable,
} from "@/services/api";
...
export default {
  ...
  mounted() {
    this.getRemainingInspectionTableData();
  },
  methods: {
    //查看图片
    ...
    editor(type_value, item) {
      ...
      this.$refs.editorDialog.afterVisibleChange(JSON.stringify(item));
    },
    getRemainingInspectionTableData() {
      let params = {
      ...
      this.spinning = true;
      getRemainingInspectionTable(params).then((res) => {
        this.spinning = false;
        console.log(res);
        if (res.status === 200) {
          ...
          for (let i = 0; i < rows.length; i++) {
            ...
            if (rows[i].taskType == 0) {
              rows[i].source = "计划生成";
            } else {
              rows[i].source = rows[i].createUserName + "生成";
            }
            ...
            rows[i].maintenanceItem = "";
            if (rows[i].cmCheckouts && rows[i].cmCheckouts.length > 0) {
              rows[i].cmCheckouts.map((cmCheckouts, index) => {
                if (index == rows[i].cmCheckouts.length - 1) {
                  rows[i].maintenanceItem += cmCheckouts.name;
                } else {
                  rows[i].maintenanceItem += cmCheckouts.name + " 、 ";
                }
              });
    ...
    getFinishedInspectionTableData() {
      let params = {
      ...
      getFinishedInspectionTable(params).then((res) => {
    ...
    search(value) {
      if (value == "0") {
        ...
        this.getFinishedInspectionTableData();
      }
    },
    refresh() {
      this.init();
