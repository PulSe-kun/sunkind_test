    ...
    <div class="footer">
      ...
      >
        ...
        <div class="dialog-content">
          ...
          <a-form-model-item label="点检周期" prop="cycle">
            <a-input
              size="large"
              placeholder="请输入点检周期"
          ...
          <a-form-model-item label="下次点检日期" prop="nextDate">
            <a-date-picker
              size="large"
              placeholder="请选择下次点检日期"
              style="width: 100%"
          ...
          <a-form-model-item label="计划描述" prop="describe">
            <a-textarea
              size="large"
...
import {
  getCmCheckBuildRule,
  editInspectionPlan,
  updateInspectionPlanMaintainItems,
} from "@/services/api";
    ...
    afterVisibleChange(val) {
      ...
      if (val) {
        ...
        this.$nextTick(function () {
          ...
          if (this.item["cmCheckouts"]) {
            this.detailData = this.item.cmCheckouts;
            this.$refs.maintain_table.getData(
              JSON.stringify(this.item.cmCheckouts)
            );
          } else {
            this.detailData = [];
          }
    ...
    onSubmit() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          console.log(params);
          editInspectionPlan(params.id, params).then((res) => {
            let { data } = res;
