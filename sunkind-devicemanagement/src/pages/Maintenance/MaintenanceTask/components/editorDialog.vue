      ...
      >
        ...
        <div style="padding: 20px" v-if="item">
          ...
          <a-form-model-item
            label="不保养原因"
            prop="note"
            v-if="type_value == '无需保养'"
          >
            <a-textarea
              size="large"
              v-model="form.note"
              placeholder="请输入不保养原因"
            />
          </a-form-model-item>
          <a-form-model-item
            label="转交给"
          ...
          <a-form-model-item
            label="备注"
            prop="notes"
            v-if="type_value == '转办' || type_value == '保养'"
          >
            <a-textarea
              size="large"
              v-model="form.notes"
          ...
          <a-form-model-item label="图片说明" v-if="type_value == '保养'">
            <a-upload
              name="avatar"
...
import {
  ...
  addActivities,
  maintainBill,
  doNothingToMaintainBill,
  transferMaintainBill,
} from "@/services/api";
...
export default {
  ...
  methods: {
    ...
    afterVisibleChange(item) {
      ...
      // console.log(this.type_value, this.item);
      this.drawerDetailData = this.item.cmItems;
      ...
      this.$refs.ruleForm.validate().then((valid) => {
        if (valid) {
              ...
              uploadImages(formData).then((res) => {
                if (res.status == 200 && res.data && res.data !== "") {
                  ...
                  addActivities(addParam).then((res) => {
                    if (res.status == 200) {
                      //保养
                      maintainBill(this.item.id).then((res) => {
                        if (res.status == 200) {
          ...
          if (this.type_value == "无需保养") {
            ...
            addActivities(addParam).then((res) => {
              if (res.status == 200) {
                doNothingToMaintainBill(this.item.id).then((res) => {
                  if (res.status == 200) {
                    this.$message.success("该保养单已无需保养！");
                    this.onClose();
                    this.$emit("refresh");
                  } else {
                    this.$message.error("无需保养失败！");
                  }
                });
              } else {
                this.$message.error("无需保养失败！");
          ...
          if (this.type_value == "转办") {
            ...
            addActivities(addParam).then((res) => {
              if (res.status == 200) {
                ...
                transferMaintainBill(this.item.id, transferParam).then(
                  (res) => {
                    if (res.status == 200) {
                      this.$message.success("该保养单已成功转办！");
                      this.onClose();
                      this.$emit("refresh");
                    } else {
