<template>
  <div class="MaintenancePlan">
    ...
    <div class="MaintenancePlan-content">
      <div class="header">
        <div>
          <ul>
            <li>
      ...
      <div class="MaintenancePlan-table">
        <div class="table-btn">
  ...
  methods: {
    ...
    setNode(data, parentId) {
      ...
      for (var i = 0; i < data.length; i++) {
        num = i;
        // console.log(data[i]);
        if (data[i].byAmount !== 0) {
          data[i].name = data[i].name + "( " + data[i].byAmount + " )";
        }
    ...
    getMaintainPlanTableData() {
      ...
      this.spinning = true;
      getMaintainPlanTable(params).then((res) => {
        console.log(res);
        this.spinning = false;
        if (res.status === 200) {
          if (res.data.rows && res.data.rows.length > 0) {
            res.data.rows.map((item) => {
              ...
              item.period =
                item.cycle +
                this.$utils.getStateName("cycleType", item.cycleType);
              item.status = this.$utils.getStateName("status", item.status);
              item.maintenanceItem = "";
              if (item.cmItems && item.cmItems.length > 0) {
                item.cmItems.map((cmItem, index) => {
                  if (index == item.cmItems.length - 1) {
                    item.maintenanceItem += cmItem.name;
                  } else {
                    item.maintenanceItem += cmItem.name + " 、 ";
                  }
                });
...
<style lang="less" scoped>
.MaintenancePlan .MaintenancePlan-content {
  overflow: auto;
}
