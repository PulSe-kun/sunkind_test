...
<script>
import { addActivities, removeMaintainBill } from "@/services/api";
      ...
      this.$refs.ruleForm.validate().then((valid) => {
        if (valid) {
          ...
          addActivities(addParam).then((res) => {
            if (res.status == 200) {
              removeMaintainBill(this.item.id).then((res) => {
                if (res.status == 200) {
                  this.$message.success("该保养单已作废！");
                  this.onClose();
                  this.$emit("refresh");
                } else {
                  this.$message.error("该保养单作废失败");
                }
              });
            } else {
              this.$message.error("该保养单作废失败");
