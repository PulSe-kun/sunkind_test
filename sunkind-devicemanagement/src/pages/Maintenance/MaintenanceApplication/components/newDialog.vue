...
<script>
import { addMaintainBill, updateBillMaintainItems } from "@/services/api";
    ...
    onSubmit() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          addMaintainBill(params).then((res) => {
            let { data } = res;
            if (data && res.status == 200) {
              if (data.id) {
                //更新关联的保养项table
                maintainItemsParam.planId = data.id;
                updateBillMaintainItems(data.id, maintainItemsParam).then(
                  (res) => {
