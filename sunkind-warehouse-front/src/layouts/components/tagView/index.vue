<template>
  <div class="tabs-head">
    ...
    <a-tabs class="tabs-container" type="editable-card" :active-key="active" :hide-add="true">
      <a-tab-pane v-for="(tab, index) in pageList" :key="tab.path">
        <div slot="tab" class="tab" @contextmenu="(e) => onContextmenu(tab.path, e)">
          <a-icon v-if="tab.path == $route.path" class="icon-sync" :type="refresh_loading ? 'loading' : 'sync'" @click="refresh" />
          <div class="title" @click="onTabClick(tab.path)">{{ tab.meta.title }}</div>
          <my-icon
            v-if="pageList.length > 1 && tab.path !== '/warehouse-management-home'"
            type="iconcha1"
...
export default {
  ...
  created() {
    /** 判断是否有标签路由 有的话从缓存里取 */
    const pageList = window.sessionStorage.getItem('pageList')
    if (pageList) {
  ...
  methods: {
    ...
    createTabs(val) {
      ...
      const children_path = val.split('/')[2]
    ...
    delRoute(index) {
      ...
      if (this.pageList[index].path == this.$route.path) {
        ...
        /** 跳转时处理菜单路由的展示 格式必须一致 防止出错 */
        window.sessionStorage.setItem('openKeys', JSON.stringify(openKeys))
        window.sessionStorage.setItem('selectedKeys', JSON.stringify([selectedKeys]))
      }
    ...
    closeLeft(pageKey) {
      ...
      let flag = true
      for (let i = 0; i < this.pageList.length; i++) {
        if (this.pageList[i].path == '/warehouse-management-home') {
          flag = false
        }
      }
      if (flag) {
