<template>
  ...
  >
    ...
    <template>
      ...
      >
        <div class="state_content">确认删除{{ form.warehouseName }}？</div>
...
<script>
import { compile_warehouse, getUserList, isDelete } from '@/services/api_basis'
export default {
  props: {
    item: {
      type: Object,
  ...
  methods: {
    ...
    afterVisibleChange(val) {
      ...
      if (val) {
        ...
        this.form.warehouseName = this.item.warehouseName
        this.form.warehouseType = this.item.warehouseType
        this.form.warehouseRemark = this.item.warehouseRemark
      }
    },
    /** 获取仓库管理员列表 */
    // getUserListData() {
    ...
    //       this.form.warehouseAdmin = this.item.warehouseAdmin
    //     }
    //   })
    // },
    delData() {
      // console.log(删除);
      isDelete(this.item.warehouseId).then(res => {
        if (res.code === 0) {
    ...
    onSubmit() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          console.log(params)
          compile_warehouse(params).then(res => {
            // console.log(res);
            if (res.code === 0) {
              this.visible = false
