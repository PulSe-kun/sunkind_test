      ...
      <div class="dialog-content">
        <div class="content-btn">
          <div>
            ...
            <a-input-search v-model="conditions" placeholder="搜索物料或托盘信息" style="width: 400px; text-align: center" @search="onSearch" />
          </div>
            ...
            <a-config-provider :locale="locale" v-if="dataList.length">
              ...
              >
                ...
                <template slot="conveyerId" slot-scope="text, item">
                  <a-select :default-value="text" style="min-width: 150px" placeholder="请选择传送线" :disabled="item.conveyer_check">
                ...
                <div slot="expandedRowRender" slot-scope="record" style="margin: 0">
                  <h3>物料信息：</h3>
                  <ul>
                    <li style="list-style: none" v-for="(matter, ind) in record.outMatterDTOS" :key="ind">
                      {{
                      matter.matterCode +
                      " " +
                      matter.matterName +
...
import { getSubStr, unit } from '@/utils/utils'
      ...
      pagePallet(params).then(res => {
        ...
        for (let i = 0; i < res.data.list.length; i++) {
          ...
          for (let j = 0; j < item.outMatterDTOS.length; j++) {
            item.outMatterDTOS[j]['_matterUnit'] = unit(item.outMatterDTOS[j].matterUnit)
          }
        }
